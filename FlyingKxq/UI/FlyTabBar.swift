...
struct FlyTabBar: View {
    ...
    func indicatorView() -> some View {
        GeometryReader { proxy in
            ...
            let offsetX = selectedIndex * (width / count) + itemWidth / 2 - barWidth / 2
            Group {
                switch type {
                case .profile:
...
enum FlyTabBarItemTypeView: View {
    case profile(text: String, selected: Bool)

    case chip(text: String, selected: Bool)

    var body: some View {
        switch self {
        case let .profile(text, selected):
            let unselectFont = UIFont.systemFont(ofSize: 14, weight: .medium)
...
struct FlyTabBarPreview: View {
    ...
    var body: some View {
        VStack {
            FlyTabBar(selectedIndex: $sliderValue, spaceAround: false, type: .chip, items: [
                "消息",
                "评论",
                "帖子",
                "收藏",
            ...
            FlyTabBar(selectedIndex: $sliderValue, spaceAround: true, type: .profile, items: [
                "消息",
                "评论",
                "帖子",
                "收藏",
